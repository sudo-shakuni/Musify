...
def get_auth_storage_path() -> str:
    ...
    if local_app_data:
        target_dir = os.path.join(local_app_data, "My-Music")
    else:
...
<head>
    <meta charset="utf-8">
    <title>My-Music — Spotify Connection Error</title>
    <style>
...
<body>
    <div class="card">
        ...
        <p>{error}</p>
        <p style="color: #777; font-size: 13px;">You may close this window and return to My-Music.</p>
    </div>
</body>
</html>"""
...
<head>
    <meta charset="utf-8">
    <title>Connected to My-Music!</title>
    <style>
        body {{ background: #121212; color: #fff; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; display: flex; align-items: center; justify-content: center; height: 100vh; margin: 0; }}
...
class SpotifyAuthManager:
    ...
    def get_liked_songs_metadata(self, limit: int = 50, offset: int = 0) -> Dict[str, Any]:
        ...
        as a standard My-Music playlist object for instant preview & 1-click download.
        """
        token = self.get_valid_token()
        if not token:
            raise ValueError("Spotify account is not linked. Please connect your account first.")
